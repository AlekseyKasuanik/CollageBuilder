...
struct GestureReducer: ReducerProtocol {
    ...
    private var collageChanger = ShapesTranslator(pointTouchSide: 0.1,
                                                  transalationStep: 0.01)
    
    mutating func reduce(_ currentState: AppState,
                         _ action: GestureAction) -> AppState {
        ...
        case .scale(let scale):
            newState = onSale(scale, in: newState)
            
        case .translate(let gestureState):
            newState = onTranslate(gestureState, in: newState)
            
        case .twoFingersTranslate(let translation):
            newState = onTwoFingersTranslate(translation, in: newState)
        }
    ...
    private mutating func onTranslate(_ gestureState: GestureState,
    ...
    private func onSale(_ scale: CGFloat,
                        in state: AppState) -> AppState {
    ...
    private func onTwoFingersTranslate(_ translation: CGPoint,
                                       in state: AppState) -> AppState {
